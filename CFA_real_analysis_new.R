  ...
  for (k in 1:length(MV_lists)){
    if (length(MV_lists[[k]])<2) {print ("only one indicator")}
      ...
      if (length(MV_lists[[k]])<3) {print ("only two indicators, cannot calculate CFA")}
        ...
        #print(MyRobustRMSEA.MLMV(fit))
        #print(summary (fit, fit.measures = TRUE ))
        #print(parameterEstimates(fit)[parameterEstimates(fit)$op == "=~",])
        #print(fitMeasures(fit,c("chisq","df","rmsea","gfi","cfi","bic","aic")))
        #print(compRelSEM(fit))
        #print(AVE(fit))
        #reliability.table[k,"Alpha"] <- round(semTools::reliability(fit)[1],3)
